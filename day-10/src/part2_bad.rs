...
fn increment_empty_around(pos: &(usize, usize), counts: &mut [Vec<i32>], map: &[Vec<Pipe>]) {
    
    let max_y = map.len() - 1;
    let max_x = map[0].len() - 1;

    for diff in AROUND {
        let pos = (bound(pos.0, max_y, diff.0), bound(pos.1, max_x, diff.1));
        if map[pos.0][pos.1] == Pipe::None {
...
fn find_enclosed_spaces(map: &Vec<Vec<Pipe>>) -> usize {

    let start = get_start_pos(map);
    let start_options = find_valid_next_pos(&start, map, true);
    let first_try_route = start_options[0];
    let mut counts = vec!(vec!(0; map[0].len()); map.len());
    increment_empty_around(&start, &mut counts, map);
    increment_empty_around(&first_try_route, &mut counts, map);
    ...
    loop {
        ...
        increment_empty_around(&cur_pos, &mut counts, map);
        //println!("Here!");
    ...
    counts.iter().flat_map(|row| row.iter()).filter(|space| **space == 8).count()
...
.....|FJLJ|FJ|F7|.LJ
....FJL-7.||.||||...
....L---J.LJ.LJLJ...", "let max_y = map.len();
    let max_x = map[0].len();8")]
